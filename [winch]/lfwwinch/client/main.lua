-- Register Events
RegisterNetEvent("lfwwinch:SetTowVehicle")
RegisterNetEvent("lfwwinch:FreeTowing")
RegisterNetEvent("lfwwinch:CreateRope")
RegisterNetEvent("lfwwinch:RemoveRope")
RegisterNetEvent("lfwwinch:LoadRopes")
RegisterNetEvent("lfwwinch:UpdateRopeLength")

-- Event handles
AddEventHandler("lfwwinch:SetTowVehicle", function(vehicle) SetTowVehicle(vehicle) end)
AddEventHandler("lfwwinch:FreeTowing", function() FreeTowing() end)
AddEventHandler("lfwwinch:CreateRope", function(netId1, netId2) CreateRopeEvent(netId1, netId2) end)
AddEventHandler("lfwwinch:RemoveRope", function(netId1, netId2) RemoveRopeEvent(netId1, netId2) end)
AddEventHandler("lfwwinch:LoadRopes", function(ropeList) LoadRopesEvent(ropeList) end)
AddEventHandler("lfwwinch:UpdateRopeLength", function(netId1, netId2, length) UpdateRopeLengthEvent(netId1, netId2, length) end)
...
-- Commands
RegisterCommand("lfwwinch", function(source, args, rawCommand)
...
function SetTowVehicle(vehicle)
    ...
    if not _g.isTowing and not _g.secondEntity then
        ...
        elseif not _g.secondEntity then
            if IsVehicleCanBeTowing(vehicle) and vehicle ~= NetworkGetEntityFromNetworkId(_g.firstEntity) then
                ...
                if distance < 20.0 then
                    ...
                    -- 开始牵引
                    TriggerServerEvent("lfwwinch:CreateRope", _g.firstEntity, _g.secondEntity)
                else
        ...
        TriggerServerEvent("lfwwinch:RemoveRope", _g.firstEntity, _g.secondEntity)
        _g.isTowing     = false
        _g.firstEntity  = nil
        _g.secondEntity = nil
    end
...
function LoadRopesEvent(ropeList)
    for k, v in pairs(ropeList) do
        if not FindRopeByNetworkId(v.netId1, v.netId2) then
            TriggerEvent("lfwwinch:CreateRope", v.netId1, v.netId2)
        end
    end
end
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('lfwwinch:LoadRopes')
    -- Main thread loop
    while true do
        -- Check if is towing
        if _g.isTowing then
    ...
    while true do
        ...
        if _g.isTowing and _g.firstEntity and _g.secondEntity then
            if _g.length and _g.length ~= lastLength then
                ...
                TriggerServerEvent("lfwwinch:UpdateRopeLength", _g.firstEntity, _g.secondEntity, _g.length)
