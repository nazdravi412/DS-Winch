-- Register Events
RegisterNetEvent("rrwinch:SetTowVehicle")
RegisterNetEvent("rrwinch:FreeTowing")
RegisterNetEvent("rrwinch:CreateRope")
RegisterNetEvent("rrwinch:RemoveRope")
RegisterNetEvent("rrwinch:LoadRopes")
RegisterNetEvent("rrwinch:UpdateRopeLength")

-- Event handles
AddEventHandler("rrwinch:SetTowVehicle", function(vehicle) SetTowVehicle(vehicle) end)
AddEventHandler("rrwinch:FreeTowing", function() FreeTowing() end)
AddEventHandler("rrwinch:CreateRope", function(netId1, netId2) CreateRopeEvent(netId1, netId2) end)
AddEventHandler("rrwinch:RemoveRope", function(netId1, netId2) RemoveRopeEvent(netId1, netId2) end)
AddEventHandler("rrwinch:LoadRopes", function(ropeList) LoadRopesEvent(ropeList) end)
AddEventHandler("rrwinch:UpdateRopeLength", function(netId1, netId2, length) UpdateRopeLengthEvent(netId1, netId2, length) end)
...
-- Commands
RegisterCommand("rrwinch", function(source, args, rawCommand)
...
function SetTowVehicle(vehicle)
    ...
    if not _g.isTowing and not _g.secondEntity then
        ...
        elseif not _g.secondEntity then
            if IsVehicleCanBeTowing(vehicle) and vehicle ~= NetworkGetEntityFromNetworkId(_g.firstEntity) then
                ...
                if distance < 20.0 then
                    ...
                    -- 开始牵引
                    TriggerServerEvent("rrwinch:CreateRope", _g.firstEntity, _g.secondEntity)
                else
        ...
        TriggerServerEvent("rrwinch:RemoveRope", _g.firstEntity, _g.secondEntity)
        _g.isTowing     = false
        _g.firstEntity  = nil
        _g.secondEntity = nil
    end
...
function LoadRopesEvent(ropeList)
    for k, v in pairs(ropeList) do
        if not FindRopeByNetworkId(v.netId1, v.netId2) then
            TriggerEvent("rrwinch:CreateRope", v.netId1, v.netId2)
        end
    end
end
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('rrwinch:LoadRopes')
    -- Main thread loop
    while true do
        -- Check if is towing
        if _g.isTowing then
    ...
    while true do
        ...
        if _g.isTowing and _g.firstEntity and _g.secondEntity then
            if _g.length and _g.length ~= lastLength then
                ...
                TriggerServerEvent("rrwinch:UpdateRopeLength", _g.firstEntity, _g.secondEntity, _g.length)
